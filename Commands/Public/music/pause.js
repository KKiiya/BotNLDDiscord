...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("pause")
    .setDescription("Pause the queue/song if unpaused."),
    /**
     * 
    ...
    execute(interaction) {
      ...
      if (executor.voice.channel != null) {
        ...
        if (!queue.paused) {
          ...
          embed.setDescription(`<@${executor.id}> has paused the queue`)
          interaction.reply({embeds: [embed], ephemeral: false })
        } else {
          embed.setTitle("Error")
          embed.setDescription(`The queue is already paused <@${executor.id}>!`)
          interaction.reply({embeds: [embed], ephemeral: false })
        }
      } else {
